...
async def test_a_placed_call_is_rekeyed_to_its_provider_id() -> None:
    """The bug that emptied the ledger.

    plan_rfq creates the row before anyone is dialled, holding "pending:{order}:{carrier}".
    Nobody wrote the real id back, so the first webhook looked it up, missed, and inserted a
    second row with no order on it. Every tool call on that call then answered "I do not have
    that operation on this call" -- the right refusal for a call the server cannot place --
    and the agent held and escalated. The conversation looked fine; nothing reached the ledger.
    """
    store = InMemoryStore()
    ledger = CallLedger(store, now=lambda: NOW)
    call_id = await store.upsert_call(
        CallRecord(
            vapi_call_id="pending:order-1:carrier-1",
            direction=CallDirection.OUTBOUND,
            phase="rfq",
            order_id="order-1",
            carrier_id="carrier-1",
    ...
    await ledger.attach_provider_id(call_id, "01a051a6-79b2-722c-b28a-c5d4f5198146")

    assert len(store.calls) == 1, "re-keying must move the row, never mint a second one"
    found = await store.call_by_vapi_id("01a051a6-79b2-722c-b28a-c5d4f5198146")
    assert found is not None
    assert found.id == call_id
    assert found.order_id == "order-1", "the operation has to survive the re-key"


async def test_rekeying_twice_changes_nothing() -> None:
    """Vapi retries. A second attach must be a no-op, not a second row."""
    store = InMemoryStore()
    ledger = CallLedger(store, now=lambda: NOW)
    call_id = await store.upsert_call(
        CallRecord(
    ...
    await ledger.attach_provider_id(call_id, "vapi-real-1")
    await ledger.attach_provider_id(call_id, "vapi-real-1")
